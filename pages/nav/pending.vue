<template>
	<view class="order-page">
		<view class="header-stickt">
			...
			<uni-nav-bar class="nav-bar-top">
				...
				<view class="nav-input-view">
					...
					<input ref="searchInput"  class="input-view" placeholder="搜索" type="text"  
		...
		<!-- 底部导航栏 -->
		<BottomNavBar ref="bottomNavRef" :modelValueFlag="1"/>
	</view>
</template>
...
<script setup>
	...
	const hideTab = () => {
		...
		kbOpen.value = true
		console.log('kbOpen', kbOpen.value)
		try { uni.hideTabBar({ animation: false }) } catch(e) {}
		// H5兜底：给body加类名隐藏DOM版tabbar
		if (typeof document !== 'undefined') document.body.classList.add('kb-open')
	}
	const showTab = () => {
		showFlag.value = true
		kbOpen.value = false
		try { uni.showTabBar({ animation: false }) } catch(e) {}
		if (typeof document !== 'undefined') document.body.classList.remove('kb-open')
	}
	...
	onUnmounted(() => {
		...
		if (typeof window !== 'undefined' && window.visualViewport && vvHandler) {
